...
pair<ll,ll> tree[4*N];
ll a[N];
pair<ll,ll> func(pair<ll,ll>&p1,pair<ll,ll>&p2)
{
    if(p1.ff==p2.ff)
        return mp(p1.ff,p1.se+p2.se);
    if(p1.ff<p2.ff)
        return p1;
    return p2;
}
...
pair<ll,ll> query(ll n,ll st,ll end,ll l,ll r)
{  
    if(l<=st && end<=r)
    return tree[n];
    ll m=(st+end)/2;
...
int main()
{
    ...
    while(t--)
    {
        ...
        fab(i,0,m)
        {
            ...
            if(c==2)
            {
                pair<ll,ll>ans=query(1,1,n,x+1,y);
                cout<<ans.ff<<" "<<ans.se<<endl;
            }
            else
                update(1,1,n,x+1,y);
        }
