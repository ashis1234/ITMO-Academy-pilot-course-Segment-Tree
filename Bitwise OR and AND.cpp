...
void update(int n,int st,int end ,int l,int r,int val)
{
    ...
    values[n] = values[2*n] & values[2*n+1];
    values[n] |= operations[n]; 
}
...
{  
    ...
    int mid = (st + end)/2;
    if(r <= mid)
        return query(2*n,st,mid,l,r) | operations[n];
    if(l > mid)
        return query(2*n+1,mid+1,end,l,r) | operations[n];

    int ans = query(2*n,st,mid,l,r) & query(2*n+1,mid+1,end,l,r);
    ans |= operations[n];
    return ans;
}
